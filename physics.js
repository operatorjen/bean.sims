...
export function computeForces(s){
  ...
  for(let i in c){let A=c[i];if(!A)continue;let{x,y,traits:t}=A,p=t.physical,m=t.mental,sp=t.spiritual,net=A.netWorth||0
    for(let j in c)if(j>i){let B=c[j];if(!B)continue;B.netWorth=w(B);let dx=B.x-x,dy=B.y-y,ds=dx*dx+dy*dy;if(!ds||!isFinite(ds))continue;let d=Math.sqrt(ds),ux=dx/d,uy=dy/d
